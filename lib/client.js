window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		Permission.VIEW_CHANNEL | Permission.SEND_MESSAGES | Permission.MANAGE_CHANNEL | Permission.CREATE_THREAD;
...
`;
		...
		*/
		...
		function RolesDialog({ open, onClose }) {
			...
			async function move(role, dir) {
				...
				await Promise.all([updateRole(role.id, { position: other.position }), updateRole(other.id, { position: role.position })]);
			}
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(TalkModal, {
				open,
				onClose,
		...
		*/
		...
		function ChatPane({ onCreateThread }) {
			...
			const canManageThread = currentThread !== null && (currentThread.createdBy === talk.me?.id || isModerator());
