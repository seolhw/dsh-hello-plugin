window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#endregion
		//#region packages/client/src/hub.ts
		...
		var HubClient = class {
			...
			async call(method, path, body, auth = false) {
				...
				if (!res.ok) {
					const err = data;
					throw new HubApiError(res.status, err?.code ?? "INTERNAL", errorMessage(err, text), err?.requestId, err?.details);
				}
				return data;
			}
		...
		async function refresh() {
			...
			try {
				...
				setState({
					...
					settings,
					me: await hub.me(),
					communities: await hub.myCommunities()
				});
			} catch (error) {
				setState({
					phase: "error",
		...
		async function registerIdentity(input) {
			...
			try {
				const hub = makeHub(await hostConfigGet());
		...
		function CommunityList({ communities }) {
			if (communities.length === 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: "还没有加入任何社区。创建/加入功能随 Hub API 完善后开启。"
			});
		...
		function TalkOverlay(_props) {
			...
			const body = talk.busy && talk.phase === "booting" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				style: { color: COLORS.muted },
				children: "连接 Hub…"
			}) : talk.phase === "error" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ErrorView, {}) : talk.phase === "anon" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(RegisterForm, { hubUrl: talk.settings?.hubUrl ?? "http://127.0.0.1:8787" }) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [
