window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function hostClone(downloadUrl) {
		...
		var ServerClient = class {
			...
			*  threadId 传讨论组 id 时返回该讨论组消息；缺省为主频道直接消息 */
			...
			renameThread(threadId, body) {
				return this.call("PATCH", `/api/threads/${threadId}`, body, true);
			}
		...
		*/
		...
		async function updateUserName(username) {
			...
			try {
				...
				const me = toUser((await server.updateUser(body)).user);
				try {
					setState({
						me,
		...
		async function updateUserAvatar(file) {
			...
			if (!url) return false;
			try {
				setState({ me: toUser((await server.updateUser({ image: url })).user) });
				notify("头像已更新");
				return true;
			} catch (error) {
				notify(errorText(error));
		...
		async function removeUserAvatar() {
			...
			if (!server) return false;
			try {
				setState({ me: toUser((await server.updateUser({ image: null })).user) });
				notify("已移除头像");
				return true;
			} catch (error) {
				notify(errorText(error));
		...
		*/
		...
		function InboxRow({ item }) {
			...
			item.kind === "invite" && item.invite?.status;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				style: rowWrap,
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Avatar, {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						children: item.kind === "invite" ? item.invite === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							style: {
								...smallText,
								fontSize: 11,
								color: palette.muted
							...
							children: "邀请已失效"
						}) : item.invite.status === "pending" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
							size: "sm",
							variant: "primary",
							disabled: busy,
							...
							disabled: busy,
							onClick: () => {
								if (window.confirm("拒绝这条社区邀请？")) declineInvite(item.invite.id);
							},
							children: "拒绝"
						})] }) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							style: {
							...
							children: STATUS_LABEL[item.invite.status] ?? item.invite.status
						}) : null
					})
				]
			});
...
`;
		...
		function CommunitiesRail({ onJoin, onCreate, onInbox, onEditProfile }) {
			const talk = useTalkState();
		...
		function ThreadListRow({ thread, channelId }) {
			const opened = useTalkState().view.threadId === thread.id;
		...
		function ThreadCreateModal({ open, onClose, channelId, seed }) {
			...
			async function submit() {
				...
				setBusy(true);
				const ok = await createThreadInChannel(channelId, seed?.starterMessageId ? {
					name: trimmed,
					starterMessageId: seed.starterMessageId
				} : { name: trimmed });
				setBusy(false);
				if (ok) onClose();
			}
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							...
							children: "把本频道最近最多 200 条消息打包成 JSON 快照，并让本机 host 流式下载到本地 `~/.dsh-talk/clones`；生成后链接也会复制到剪贴板，可分享给其它人。"
						})
					]
				})
			});
